...
public class CacheConfig implements JCacheManagerCustomizer {
    ...
    public void customize(CacheManager cacheManager) {
        ...
        try {
            ...
            LOGGER.info("Greide ikke å opprette cache {}. Fins den fra før? {}", ALTINN_RIGHTS_CACHE, e.getMessage(), e);
        }
        try {
            cacheManager.createCache(ALTINN_ROLES_CACHE, configurationTenMinutes);
        } catch (CacheException e) {
            LOGGER.info("Greide ikke å opprette cache {}. Fins den fra før? {}", ALTINN_ROLES_CACHE, e.getMessage(), e);
        }
